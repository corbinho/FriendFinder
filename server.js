...
app.post("/api/friends", function (req, res) {
    ...
    for (i = 0; i < friendsList.friends.length - 1; i++) {
        ...
        if (totalDifference < lowestScore) {
            ...
            console.log("lowest score: " + lowestScore + "from " + newBestFriendName)

        } else {
            console.log(friendsList.friends[i].name + " is not your best friend yet")
        }
    }
    
    console.log("your total difference score was " + totalDifference + " from " + newBestFriendName)
    res.json({
